...
<h1>attendee Form</h1>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO workshop (Workshop_ID,Title,Instructor_ID) VALUES (?, ?, ?)");
        $stmt->bind_param("iss", $Workshop_ID,$Title,$Instructor_ID);

        // Set parameters from POST data with validation (optional)
        $Workshop_ID = htmlspecialchars($_POST['Workshop_ID']); // Prevent XSS
        $Title = intval($_POST['Title']); // Ensure integer for ID
    ...
    <table border="1">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $Workshop_ID = $row['Workshop_ID']; // Fetch the Id
                echo "<tr>
                    <td>" . $row['Workshop_ID'] . "</td>
                    <td>" . $row['Title'] . "</td>
                    <td>" . $row['Instructor_ID'] . "</td>
                    <td><a style='padding:4px' href='delete_workshop.php?Workshop_ID=$Workshop_ID'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_workshop.php?Workshop_ID=$Workshop_ID'>Update</a></td> 
                </tr>";
            }
        } else {
