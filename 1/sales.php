...
<h1>sales Form</h1>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="number" id="Sale_ID" name="Sale_ID"><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO sales (Sale_ID,Workshop_ID,Attendee_ID) VALUES (?, ?, ?)");
        $stmt->bind_param("iii", $Sale_ID,$Workshop_ID,$Attendee_ID);

        // Set parameters from POST data with validation (optional)
        $Sale_ID = intval($_POST['Sale_ID']); // Ensure integer for ID
    ...
    <table border="1">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $Sale_ID = $row['Sale_ID']; // Fetch the Id
                echo "<tr>
                    <td>" . $row['Sale_ID'] . "</td>
                    <td>" . $row['Workshop_ID'] . "</td>
                    <td>" . $row['Attendee_ID'] . "</td>
                    <td><a style='padding:2px' href='delete_sales.php?Sale_ID=$Sale_ID'>Delete</a></td> 
                    <td><a style='padding:2px' href='update_sales.php?Sale_ID=$Sale_ID'>Update</a></td> 
                </tr>";
            }
        } else {
