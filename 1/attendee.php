...
<h1>attendee Form</h1>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO Attendee (Attendee_ID,Workshop_ID,Name) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $Attendee_ID,$Workshop_ID,$Name);

        // Set parameters from POST data with validation (optional)
        $Attendee_ID = intval($_POST['Attendee_ID']); // Ensure integer for ID
    ...
    <table border="1">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $Attendee_ID = $row['Attendee_ID']; // Fetch the Id
                echo "<tr>
                    <td>" . $row['Attendee_ID'] . "</td>
                    <td>" . $row['Workshop_ID'] . "</td>
                    <td>" . $row['Name'] . "</td>
                    <td><a style='padding:2px' href='delete_attendee.php?Attendee_ID=$Attendee_ID'>Delete</a></td> 
                    <td><a style='padding:2px' href='update_attendee.php?Attendee_ID=$Attendee_ID'>Update</a></td> 
                </tr>";
            }
        } else {
