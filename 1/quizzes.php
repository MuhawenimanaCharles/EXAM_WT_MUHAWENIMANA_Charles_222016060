...
<h1>QUIZZES Form</h1>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO quizzes (Quiz_ID,Resource_ID,Title) VALUES (?, ?, ?)");
        $stmt->bind_param("iis", $Quiz_ID,$Resource_ID,$Title);

        // Set parameters from POST data with validation (optional)
        $Quiz_ID = intval($_POST['Quiz_ID']); // Ensure integer for ID
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td>" . $row['Title'] . "</td>
                    <td><a style='padding:2px' href='delete_quizzes.php?Quiz_ID=$Quiz_ID'>Delete</a></td> 
                    <td><a style='padding:2px' href='update_quizzes.php?Quiz_ID=$Quiz_ID'>Update</a></td> 
                </tr>";
            }
        } else {
