...
<h1>instructor Form</h1>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO instructor (Instructor_ID,Name,Email) VALUES (?, ?, ?)");
        $stmt->bind_param("iss", $Instructor_ID,$Name,$Email);

        // Set parameters from POST data with validation (optional)
        $Instructor_ID = intval($_POST['Instructor_ID']); // Ensure integer for ID
...
?>
<h2>Data for Instructors Form</h2>
    <table border="1">
        <tr>
            <th>Instructor ID</th>
            <th>Name</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td>" . $row['Name'] . "</td>
                    <td>" . $row['Email'] . "</td>
                    <td><a style='padding:2px' href='delete_instructor.php?Instructor_ID=$Instructor_ID'>Delete</a></td> 
                    <td><a style='padding:2px' href='update_instructor.php?Instructor_ID=$Instructor_ID'>Update</a></td> 
                </tr>";
            }
        } else {
